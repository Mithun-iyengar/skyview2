#!/usr/bin/env python3
"""
Test Seat Locking Scenario
Demonstrates how seats get blocked when multiple users try to book simultaneously
"""
...
def book_seat(user_name, token, user_id, email, flight_id, seat_number):
...
def user_a_booking_flow(flight_id, seat_to_book):
    ...
    print("\n" + "="*70)
    print("USER A - BOOKING FLOW")
    print("="*70)
    
    try:
        token_a, user_id_a, email_a = register_and_login()
        print(f"✅ USER A registered and logged in (ID: {user_id_a})")
        ...
        # Book seat
        print(f"🔄 USER A attempting to book seat {seat_to_book}...")
        status, body = book_seat("A", token_a, user_id_a, email_a, flight_id, seat_to_book)
        ...
        if status in (200, 201):
            ...
            print(f"❌ USER A booking FAILED (Status: {status})")
            print(f"   Response: {body}")
            RESULTS["user_a"]["booking_id"] = None
...
def user_b_booking_flow(flight_id, seat_to_book):
    ...
    print("\n" + "="*70)
    print("USER B - BOOKING FLOW (SAME SEAT)")
    print("="*70)
    ...
    try:
        ...
        print(f"✅ USER B registered and logged in (ID: {user_id_b})")
        ...
        status, body = book_seat("B", token_b, user_id_b, email_b, flight_id, seat_to_book)
        ...
        if status in (200, 201):
            ...
            print(f"   Error: {error_msg.get('message', 'Seat not available')}")
...
def main():
    ...
    print("╔" + "="*68 + "╗")
    print("║" + " "*68 + "║")
    print("║" + "  SEAT LOCKING TEST - CONCURRENT BOOKING SCENARIO".center(68) + "║")
    print("║" + " "*68 + "║")
    print("╚" + "="*68 + "╝")
    ...
    try:
        ...
        print(f"✅ Flight created/retrieved (ID: {flight_id}, Number: {flight.get('flightNumber')})")
        ...
        print("\n" + "="*70)
        print("RUNNING CONCURRENT BOOKING SCENARIO")
        print("="*70)
        ...
        print("\n📊 USER A:")
        print(f"   ID: {RESULTS['user_a'].get('user_id')}")
        print(f"   Seat Status Before: {RESULTS['user_a'].get('seat_status_before')}")
        print(f"   Booking Status Code: {RESULTS['user_a'].get('booking_status_code')}")
        print(f"   Booking Status: {RESULTS['user_a'].get('booking_status')}")
        print(f"   Seat Status After: {RESULTS['user_a'].get('seat_status_after')}")
        
        print("\n📊 USER B (Tried to book same seat):")
        print(f"   ID: {RESULTS['user_b'].get('user_id')}")
        print(f"   Seat Status Before: {RESULTS['user_b'].get('seat_status_before')}")
        print(f"   Booking Status Code: {RESULTS['user_b'].get('booking_status_code')}")
        ...
        print("\n" + "="*70)
        print("✅ TEST COMPLETE - Seat locking is working!")
        print("="*70 + "\n")
